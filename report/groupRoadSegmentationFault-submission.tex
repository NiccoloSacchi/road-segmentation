...
\section{Results}
\label{sec:results}
	The traditional CNN was trained for 1000 epochs, each consisting of 25 batches of 4 images, on a NVIDIA GeForce GTX graphics card with 2 GB of VRAM, obtaining an F1 score of 83.3\%. While the U-Net was trained for 10000 iteration with a learning rate $\lambda = 10^{-3}$ which lead to an F1 score of 93.85\% on the test set submitted on Kaggle.
	...
	\begin{figure}[h]
		...
		\label{fig:pr}
	\end{figure}
	We can observe in this plot three important things: (1) the yellow curve is bellow every others, meaning that U-Net is performing better than the traditional approach, (2) the green curve is above the red one, i.e. adding more highway images and getting rid off the sea sides images improved the training, (3) in some cases the blue curve is above the green one. In fact, when we are not rotating any image, the network learns better how to detect verticals and horizontals lines but for the others the result is worst. That explains why the blue curve goes above the yellow one between 0.6 and 0.8. We can conclude that the yellow curve corresponds to the best model, that is the final model we used in the Kaggle competition.
\section{Conclusion}
...
Even without any preprocessing other than data augmentation both the models outperformed the baselines and confirmed their superiority in solving this kind of tasks.
