...
trainer.load(num_iter)
#-------------------------------------------------------------------------------

# compute and save the loss graph
#-------------------------------------------------------------------------------
loss = trainer.summary.get("training.loss")
plt.figure()
yhat = savgol_filter(loss[1], 101, 3)
plt.plot(loss[0],yhat)
plt.title('Objective function')
plt.ylabel('loss')
plt.xlabel('iteration')
plt.savefig(save_path+'/loss.png')
logger.info('loss_.png is saved')
#-------------------------------------------------------------------------------
...
for im in tests:
    ...
    var  = Variable(torch.from_numpy(np.ascontiguousarray(im)).cuda())
    pred = unet_clsf(var)
    pred = pred.cpu().data.numpy()
    print(save_path+'/satImage_' + '%.3d' % i + '.png')
    plt.imsave(save_path+'/prediction/satImage_' + '%.3d' % i + '.png',pred[0,1,:])
    i = i + 1
#-------------------------------------------------------------------------------

# computing the submission
#-------------------------------------------------------------------------------
submission_filename = save_path+'/submission.csv'
image_filenames = []
logger.info('computing the submission : ')
for i in range(1, 51):
    image_filename = save_path+'/prediction/satImage_' + '%.3d' % i + '.png'
